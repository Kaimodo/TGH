...
task AnalyzeModuleRelease -if {$Script:BuildEnv.OptionAnalyzeCode} {
    ...
    $StageReleasePath = Join-Path (Join-Path $BuildRoot $Script:BuildEnv.ScratchFolder) $Script:BuildEnv.BaseReleaseFolder
    $Analysis = Invoke-ScriptAnalyzer -Path $StageReleasePath -ExcludeRule $Script:BuildEnv.ExcludeRules
...
task PublishPSGallery LoadBuildTools, InstallModule, {
    ...
    if (Test-Path $CurrentModule) {
        ...
        Write-Description White "Uploading project to $($Script:BuildEnv.PSRepository): $($Script:BuildEnv.ModuleToBuild)"
        Upload-ProjectToPSGallery -Name $Script:BuildEnv.ModuleToBuild -Repository $Script:BuildEnv.PSRepository -NuGetApiKey $Script:BuildEnv.NuGetApiKey
    }
