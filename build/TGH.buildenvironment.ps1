...
if ((Get-Variable 'BuildEnv' -ErrorAction:SilentlyContinue) -eq $null) {
    $Script:BuildEnv = New-Object -TypeName PSObject -Property @{
        ...
        ModuleToBuild = 'TGH'
        ModuleVersion = '0.0.1'
        ModuleWebsite = 'https://www.github.com/Kaimodo/TGH'
        ModuleCopyright = "(c) $((get-date).Year.ToString()) Kai Krutscho. All rights reserved."
        ModuleLicenseURI = 'https://www.github.com/Kaimodo/TGH/LICENSE.md'
        ModuleTags = 'Server, PrivateUse' -split ','
        ModuleAuthor = 'Kai Krutscho'
        ModuleDescription = 'Module for Private Server use'
        ...
        #Extra-Options
        ExcludeRules  = "PSAvoidUsingUserNameAndPassWordParams,PSAvoidUsingConvertToSecureStringWithPlainText" -split ','
        ...
        # You should NOT enter this key here but rather manually enter it in the TGH.buildenvironment.json file with: set-buildenvironment -NugetAPIKey '<key>'
    ...
    ########################################
    $PersistentBuildFile = join-path $PSScriptRoot "TGH.buildenvironment.json"
